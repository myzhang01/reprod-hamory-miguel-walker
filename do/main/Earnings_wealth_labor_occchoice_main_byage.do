 ...
 * Table: Table 2
 * Description: The main earnings, labor supply, and occupational choice results
...
//Initialize .tex file
	texdoc init "$output/KLPS4_E+_earnings_labor_occchoice_main_byage.tex", replace force
	texdoc write \\ \toprule
	texdoc close
...
//Column 1: Pooled other earnings outcomes - ALL
...
foreach outcome of local outcomes {
	...
	if "`outcome'" == "tot_wealth_t" {
		...
		eststo model`i': reg `outcome' treatment $x_controls_panel [pw=weight], cluster(psdpsch98)
	}
}
...
foreach outcome of local outcomes {
	...
	if "`outcome'" == "tot_wealth_t" {
		...
		eststo model`i': reg `outcome' treatment treatXfemale costXfemale satXfemale  $x_controls_panel [pw=weight], cluster(psdpsch98)
	}
}
...
foreach outcome of local outcomes {
	...
	if "`outcome'"=="tot_wealth_t" {
		...
		eststo model`i': reg `outcome' younger treatment treatXyounger costXyounger satXyounger  $x_controls_panel [pw=weight], cluster(psdpsch98)
	}
}
...
texdoc init "$output/KLPS4_E+_earnings_labor_occchoice_main_byage.tex", append force
...
foreach outcome of local outcomes {
	local ++i
	eststo model`i': reg `outcome' treatment treatXfemale satXfemale costXfemale $x_controls_panel [pw=weight], cluster(psdpsch98)
}
...
foreach outcome of local outcomes {
	local ++i
	eststo model`i': reg `outcome' younger treatment treatXyounger satXyounger costXyounger $x_controls_panel [pw=weight], cluster(psdpsch98)
}
