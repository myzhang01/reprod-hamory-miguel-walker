 ...
 * Filename: Earnings_wealth_labor_occchoice_main_byage_scyvoced.do
 * Table: Appendix Table S6
 ...
 * 		using KLPS-2, KLPS-3, and KLPS-4 data including those in SCY/VocEd.
...
//Initialize Latex document
	texdoc init "$output/KLPS4_E+_earnings_labor_occchoice_main_byage_scyvoced.tex", replace force
	texdoc write \\ \toprule
	texdoc close
...
foreach outcome of local outcomes {
	local ++i
	if "`outcome'" == "t_wealth_loc_mp" {
		eststo model`i': reg `outcome' treatment voced_treat_voucher scy_treat $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
	}
	else {
		eststo model`i': reg `outcome' treatment voced_treat_voucher scy_treat $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	}
}
...
foreach outcome of local outcomes {
	local ++i
	if "`outcome'" == "t_wealth_loc_mp" {
		eststo model`i': reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale  $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
	}
	else {
		eststo model`i': reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale  $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	}
}
...
foreach outcome of local outcomes {
	local ++i
	if "`outcome'"=="t_wealth_loc_mp" {
		eststo model`i': reg `outcome' younger treatment voced_treat_voucher scy_treat treatXyounger costXyounger satXyounger  $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
	}
	else {
		eststo model`i': reg `outcome' younger treatment voced_treat_voucher scy_treat treatXyounger costXyounger satXyounger  $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	}
}
...
foreach outcome of local outcomes {
	...
	summ `outcome' if treatment == 0 [aw=weight_scyvoced] 
	matrix tmp = (r(mean))
...
//Output table
# delimit ;
esttab using "output/KLPS4_E+_earnings_labor_occchoice_main_byage_scyvoced.tex", append 
...
texdoc init "$output/KLPS4_E+_earnings_labor_occchoice_main_byage_scyvoced.tex", append force
...
foreach outcome of local outcomes {
	local ++i
	eststo model`i': reg `outcome' treatment treatXfemale satXfemale costXfemale voced_treat_voucher scy_treat $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
}
...
foreach outcome of local outcomes {
	local ++i
	eststo model`i': reg `outcome' younger treatment treatXyounger satXyounger costXyounger voced_treat_voucher scy_treat $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
}
