 ...
 * Filename: Attrition_byage.do
 * Table: Appendix Table S2
 * Description: This do file serves as the do file to generate
 * 		the multi-panel results by older/younger subsample
 *		for KLPS-2, KLPS-3, and KLPS-4 attrition.
...
//Define locals
	...
	*Samples
	local genders full_sample older younger

	*Column numbers (samples * 2)
	...
	*Column names
	...
	local colname_older "Older"
	local colname_younger "Younger"

//Initialize latex file
	*Create latex file
	texdoc init "$output/KLPS4_E+_Attrition_olderyounger.tex", replace force
...
foreach panel of local panels {
	...
	*Drop those with missing year at baseline
	replace full_sample = . if older==.
	...
	//Run regressions on treatment - control differences	
		...
		*Panel and variable labels
		...
		//Output table
		# delimit ;
			esttab using "output/KLPS4_E+_Attrition_olderyounger.tex", 
...
//Close out latex file
	texdoc init "$output/KLPS4_E+_Attrition_olderyounger.tex", append force
