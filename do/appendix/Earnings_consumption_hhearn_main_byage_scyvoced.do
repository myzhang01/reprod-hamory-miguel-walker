 ...
 * Filename: Earnings_consumption_hhearn_main_byage_scyvoced.do
 * Table: Appendix Table S5
 * Description: This do file creates the table which shows the treatment effect 
 *		on consumption, earnings, and household earnings using KLPS-2, KLPS-3
 * 		and KLPS-4 data, including those in SCY/VocEd
...
//Initialize Latex document
	texdoc init "$output/KLPS4_E+_earnings_consumption_main_byage_scyvoced.tex", replace force
	texdoc write \\ \toprule
	texdoc close
...
//Use data and append SCY/VocEd observations
use "$data/Worms20_Analysis.dta", clear
append using "$data/Worms20_Analysis_scyvoced.dta"

replace voced_treat_voucher = 0 if voced_treat_voucher==.
replace scy_treat = 0 if scy_treat==.
...
//Define outcomes used in FDR adjustment (consumption, earnings)
local outcomes_FDR tot_cnsp_t tot_earn12m_t

foreach outcome of local outcomes_FDR {
	//Full sample
		use `all_data',clear
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	...
	//Females
		use `all_data',clear
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat treatXmale costXmale satXmale cost_sharing saturation_dm  avgtest96 pup_pop demeaned_popT_6k zoneidI2-zoneidI8  male std98_base_I2-std98_base_I6 month_interviewI2-month_interviewI12 wave2 i.year voced [pw=weight_scyvoced], cluster(psdpsch98)
	...
	//Males 
		use `all_data',clear
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	...
	//Older
		use `all_data',clear
		drop if older==.
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat treatXyounger costXyounger satXyounger younger $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	...
	//Younger
		use `all_data',clear
		drop if older==.
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat treatXolder costXolder satXolder older $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
...
foreach outcome in tot_cnsp_t{
	...
	*COL 1/5: ALL
		sum `outcome' if treatment==0 [aw=weight_scyvoced]
	...
	*COL 2/5: FEMALE
		sum `outcome' if treatment==0 & female==1 [aw=weight_scyvoced]
		local controlMean = r(mean)

		reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
		...
		ereturn post b [aw=weight_scyvoced], depname(treatment)
		estadd matrix se
		eststo
	...
	*COL 3/5: MALE
		sum `outcome' if treatment==0 & female==0 [aw=weight_scyvoced]
		local controlMean = r(mean)
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	...
	*COL 4/5: OLDER
		sum `outcome' if treatment==0 & older==1 [aw=weight_scyvoced]
		local controlMean = r(mean)
		
		reg `outcome' older treatment voced_treat_voucher scy_treat treatXolder costXolder satXolder $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
	...
	*COL 5/5: YOUNGER
		sum `outcome' if treatment==0 & older==0 [aw=weight_scyvoced]
		local controlMean = r(mean)
		eststo: reg `outcome' older treatment voced_treat_voucher scy_treat treatXolder costXolder satXolder $x_controls_panel [pw=weight_scyvoced], cluster(psdpsch98)
...
esttab using "output/KLPS4_E+_earnings_consumption_main_byage_scyvoced.tex", append
	keep(treatment)
...
texdoc init "$output/KLPS4_E+_earnings_consumption_main_byage_scyvoced.tex", append force
...
foreach outcome in tot_hhearn12m_t  {
	...
	*COL 1/5: ALL
		...
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
	...
	*COL 2/5: FEMALE
		...
		reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
		count if e(sample) & female==1
		local numobs=r(N)
	...
	*COL 3/6: MALE
		sum `outcome' if treatment==0 & female==0 [aw=weight_scyvoced]
		local controlMean = r(mean)
		eststo: reg `outcome' treatment voced_treat_voucher scy_treat treatXfemale costXfemale satXfemale $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
	...
	*COL 4/5: OLDER
		...
		reg `outcome' older treatment voced_treat_voucher scy_treat treatXolder costXolder satXolder $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
		count if e(sample) & older==1
		local numobs=r(N)
	...
	*COL 5/5: YOUNGER
		sum `outcome' if treatment==0 & older==0 [aw=weight_scyvoced]
		local controlMean = r(mean)
		eststo: reg `outcome' older treatment voced_treat_voucher scy_treat treatXolder costXolder satXolder $x_controls1 [pw=weight_scyvoced], cluster(psdpsch98)
